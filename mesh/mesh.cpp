                ...
                bool fix_orientation)
{
   ...
   else if (mesh_type == "$MeshFormat")
   {
     ...
     getline(input, buff);
     // There is additional number in binary format
     if (binary)
     {
       int one;
     ...
     // get an important info about mesh dimension and number of elements
//     streampos pos_for_returning = input.tellg();
//     get_info_gmsh_mesh(input, Dim, NumOfElements, NumOfBdrElements);
//     input.clear();
//     input.seekg(pos_for_returning);

//     elements.SetSize(NumOfElements);
//     boundary.SetSize(NumOfBdrElements);

     // A map between a serial number of the vertex and its number in the
     // file (there may be gaps in numeration)
           ...
           while (n_elem_part < num_of_all_elements)
           {
             ...
             for (int el = 0; el < n_elem_one_type; ++el)
             {
               ...
               for (int vi = 0; vi < n_elem_nodes; ++vi)
                 vert_indices[vi] = vertices_map.find(data[1+n_tags+vi])->second;
         ...
         else // ASCII
         {
           for (int el = 0; el < num_of_all_elements; ++el)
           {
             ...
             for (int vi = 0; vi < n_elem_nodes; ++vi)
             {
               int index;
               input >> index;
               vert_indices[vi] = vertices_map.find(index)->second;
             }
             // initialize the mesh element
             switch (type_of_element)
             {
